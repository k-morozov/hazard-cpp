...
class Runner {
   public:
    explicit Runner(uint64_t num_iterations)
    ...
    void Do(Function&& func, Args&&... args) {
        threads_.emplace_back(
            [this](std::remove_reference_t<Function>&& func,
                ...
                while (iteration_.fetch_add(1, std::memory_order::relaxed) <
                       num_iterations_) {
...
class TimeRunner {
   public:
    explicit TimeRunner(std::chrono::steady_clock::duration time)
        : time_{time} {
    ...
    template <class Function, class... Args>
    void Do(Function&& func, Args&&... args) {
        DoWithInit([] {}, std::forward<Function>(func),
